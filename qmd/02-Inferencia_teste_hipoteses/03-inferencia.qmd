...
Funções: <a href="https://github.com/FCopf/amostragem-inferencia/blob/main/scripts/normal_empirica_gg.r" target="_blank">normal_empirica_gg.r</a>
...
Se queremos um nível de confiança diferente, basta ajustar o valor de $\alpha$. Por exemplo, se queremos um nível de significância a 99%, fixamos $\alpha$ em $0.01$ e portanto $z = 2.58$. Da mesma forma, o $IC_{90\%}$ poderá ser obtido com $\alpha = 0.10$ e consequentemente $z = 1.64$. Estes e outros limites descrevem as probabilidades em uma distribuição normal padronizada (@sec-norm), que podem ser obtidos com o uso da maioria dos softwares estatísticos, além de estarem incluso na <a href="https://github.com/FCopf/amostragem-inferencia/blob/main/qmd/02-Inferencia_teste_hipoteses/img/Tabela_Z.pdf" target="_blank">Tabela Z</a>, encontrada na grande maioria dos livros de estatística básica. Os valores de $Z$ são os mesmos discutidos no tópico <a href="https://fcopf.github.io/visualizacao-de-dados/qmd/02_dados_univariados/06-escorez.html" target="_blank">Medidas de posição</a>.
...
Uma representação esquemática do erro amostralPara o cálculo do intervalo de confiança, estamos assumindo que as médias amostrais têm **Distribuição Normal** com média $\mu$ e desvio padrão $\frac{\sigma}{\sqrt{n}}$. Fazendo isto, estamos no **Teorema Central do Limite (TCL)** (@sec-tcl). Geralmente não temos os valores de $\mu$ e $\sigma$, de modo que utilizamos os valores de $\overline{X}$ e $s$ calculados a partir de nossa amostra. Quando as amostras são grandes ($n\ge{30}$) não há problema em utilizar o valor de $z_{\alpha/2}$, e assim:
