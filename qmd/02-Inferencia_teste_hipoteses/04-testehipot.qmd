...
A probabilidade $P(Z \ge z)$ é encontrada na **distribuição normal padronizada** (@sec-norm) em que $Z \sim \mathcal{N}(0,\,\frac{\sigma}{\sqrt{n}})$ e como nossa pergunta se refere à **distância** entre $\overline{X}$ e $\mu$, devemos encontar também $P(Z \le -z)$, de modo que a probabilidade que nos interessa é denominada de **valor de p** de um teste de hipóteses:
...
e utilizando a <a href="https://github.com/FCopf/amostragem-inferencia/blob/main/files/Tabela_Z.pdf" target="_blank">Tabela Z</a> , encontramos a probabilidade de obtermos valores tão ou mais extremos que $-`r round(z,2)`$ e $+`r round(z,2)`$.
